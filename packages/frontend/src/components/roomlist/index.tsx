 ...
 *
 * - lists game rooms (grid)
 * - open details modal for a room (fetches /game/info/:id)
 * - supports "connect" flow demo (calls /game/connect/:gameId/:playerId)
 *
 * Note: actual join flow depends on your backend endpoints (create/join). This component only
 * shows how to call prepareConnectApi once you have a playerId.
 */
export function RoomList() {
  ...
  async function handlePrepareConnect(gameId: string) {
    // NOTE: prepareConnectApi requires a playerId. In a real flow you will either:
    //  - call an endpoint to join/create a player and get playerId, OR
    //  - request the user to input a player name and call an appropriate join API that returns playerId.
    //
    // Here we just demo calling prepareConnectApi with a placeholder playerId (NOT for production).
    ...
    try {
      ...
      // Optionally navigate to a "connect" route or open WS using resp.data.domains
      // e.g. navigate(`/game/connect/${gameId}/${playerId}`) or open WS client
    } catch (err: any) {
  ...
  return (
    <div class="container mx-auto p-4">
      ...
      <Show when={gamesQuery.isLoading} fallback={
