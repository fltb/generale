...
import { createMapApi, updateMapApi, mapDetailApi, uploadMapThumbnailApi, discardDraftApi, mapThumbnailUrl } from '~/api/mapApi';
...
export default function MapEditor(props: MapEditorProps) {
  ...
  let [thumbnailBusy, setThumbnailBusy] = createSignal(false);
  let [thumbStamp, setThumbStamp] = createSignal(0);
  ...
  function resizeMap(nw: number, nh: number) {
    ...
    setWidth(newW); setHeight(newH); setTiles(result);
  }
  ...
  function handlePointerDown(e: any) {
    ...
    if (cell) { setIsPainting(true); paintCell(cell.x, cell.y); }
  }
  ...
  function handlePointerMove(e: any) {
    ...
    const cell = screenToWorld(e.globalX, e.globalY);
    if (!cell) return;
    const t = tiles(), cur = t[cell.y]?.[cell.x];
    const type = selectedType();
    const army = PASSABLE_TYPES.has(type) ? selectedArmy() : 0;
  ...
  function handlePointerUp(e: any) {
    if (e.button === 1 || e.button === 2) { setIsDragPan(false); return; }
    setIsPainting(false);
  }
  ...
  async function generatePreview() {
    ...
    if (!id) { setMessage('请先保存地图'); return; }
    ...
    try {
      ...
      for (let y = 0; y < h; y++) {
        ...
        for (let x = 0; x < w; x++) {
          ...
          const bgColor = cell.type === TileType.Fog
          ...
          if (iconKey) {
            ...
            const icon = iconFactory.createScaledIcon(iconKey as any, iconSize, iconColorNum);
  ...
  async function uploadThumbnail(e: Event) {
    ...
    if (!file || !editingMapId()) return;
    setThumbnailBusy(true);
    try { await uploadMapThumbnailApi(editingMapId()!, file); setMessage('封面上传成功'); hasCustomThumbnail = true; setThumbStamp(Date.now()); }
    catch (e: any) { setMessage(`上传失败: ${e.message}`); }
    finally { setThumbnailBusy(false); }
    input.value = '';
  }
  ...
  async function loadMap(id: string) {
    try {
      ...
      setTiles((d.tiles || []).map((row: any[]) => row.map((cell: any) => ({
        type: (cell.type as TileType) ?? TileType.Plain,
        army: typeof cell.army === 'number' ? cell.army : 0,
      }))));
      setEditingMapId(id);
      setHasDraft(!!(d as any).hasDraft);
      hasCustomThumbnail = !!(d as any).hasCustomThumbnail;
    } catch (e: any) { setMessage(`加载失败: ${e.message}`); }
  }
  ...
  async function discardDraft() {
    ...
    } catch (e: any) { setMessage(`放弃草稿失败: ${e.message}`); }
  }

  onMount(async () => {
    if (props.mapId) await loadMap(props.mapId);
    else if (props.cloneFromId) { await loadMap(props.cloneFromId); setEditingMapId(null); }
  ...
  return (
    <div class="flex h-full">
      <div class="w-56 bg-base-200 p-3 flex flex-col gap-2 shrink-0">
        <A href="/maps" class="text-xs opacity-50 hover:opacity-100 mb-1">← 返回地图工坊</A>

        <div><div class="text-xs font-medium mb-0.5">地图名称</div>
          <Input value={name()} onInput={(e) => setName(e.currentTarget.value)} placeholder="输入地图名称" size="sm" /></div>
        <div><div class="text-xs font-medium mb-0.5">描述</div>
          <Input value={description()} onInput={(e) => setDescription(e.currentTarget.value)} placeholder="可选描述" size="sm" /></div>

        <div class="flex gap-2">
          <div class="flex-1"><div class="text-xs font-medium mb-0.5">宽</div>
            <Input type="number" value={String(width())} onInput={(e) => resizeMap(Number(e.currentTarget.value), height())} size="sm" /></div>
          <div class="flex-1"><div class="text-xs font-medium mb-0.5">高</div>
            <Input type="number" value={String(height())} onInput={(e) => resizeMap(width(), Number(e.currentTarget.value))} size="sm" /></div>
        </div>

        <div><div class="text-xs font-medium mb-0.5">地形类型</div>
          <div class="grid grid-cols-2 gap-1">
            {TILE_TYPES.map((t) => (
              <button
                class={`px-2 py-1 text-xs rounded border font-semibold cursor-pointer ${
                  selectedType() === t ? 'ring-2 ring-primary border-primary' : 'border-base-300 hover:border-base-content/30'
                }`}
                style={{ 'background-color': TILE_COLORS[t] ?? '#ccc', 'color': '#111', 'text-shadow': '0 0 2px rgba(255,255,255,0.6)' }}
                onClick={() => setSelectedType(t)}
              >{TILE_LABELS[t]}</button>
            ))}
          </div>
        </div>

        <div><div class="text-xs font-medium mb-0.5">兵力</div>
          <Show when={PASSABLE_TYPES.has(selectedType())} fallback={
            <div class="text-xs opacity-50">该地形不可驻兵</div>
          }>
            <Input type="number" value={String(selectedArmy())} onInput={(e) => setSelectedArmy(Number(e.currentTarget.value))} size="sm" />
          </Show>
        </div>
        ...
        <Show when={editingMapId()}>
          <div class="border-t border-base-300 pt-2 flex flex-col gap-2">
            ...
            <Show when={mapThumbnailUrl(editingMapId()!)}>
              <img src={`${mapThumbnailUrl(editingMapId()!)}?v=${thumbStamp()}`}
                alt="预览" class="w-full rounded border border-base-300" />
            </Show>
            ...
            <label class="cursor-pointer">
              ...
              <input type="file" accept="image/png,image/jpeg,image/webp" class="hidden" onChange={uploadThumbnail} disabled={thumbnailBusy()} />
            </label>
        ...
        <div class="flex flex-col gap-2 mt-4 pt-2 border-t border-base-300">
          <Button variant="ghost" size="sm" onClick={() => save(false)} disabled={saving() || !name()}>{saving() ? '保存中...' : '保存草稿'}</Button>
          <Show when={hasDraft()}>
            <Button variant="ghost" size="xs" onClick={discardDraft}>放弃草稿</Button>
          </Show>
          <Button variant="primary" size="sm" onClick={() => save(true)} disabled={saving() || !name()}>{saving() ? '发布中...' : '发布地图'}</Button>
          {message() && <div class={`text-xs ${message().startsWith('保存失败') || message().startsWith('加载失败') ? 'text-error' : 'text-success'}`}>{message()}</div>}
        </div>
      </div>

      <div class="flex-1 relative overflow-hidden bg-base-300" ref={containerRef} onContextMenu={(e) => e.preventDefault()}>
        <P.Application ref={(r: any) => { appRef = r; }} background={DEFAULT_TILE_THEME.colors.appBackground} antialias>
          <P.Container interactive onpointerdown={handlePointerDown} onpointermove={handlePointerMove} onpointerup={handlePointerUp} hitArea={{ contains: () => true }}>
            <P.Container x={viewX()} y={viewY()} scale={viewScale()}>
              <Show when={tiles().length > 0}>
                {tiles().map((row, y) => row.map((cell, x) => (
                  <MapTile coord={{ x, y }} tile={{ type: cell.type, ownerId: null, army: cell.army }} size={GRID_CELL}
                    playerDisplay={EMPTY_DISPLAY} iconTextures={DEFAULT_TILE_THEME.tileIcon} iconFactory={iconFactory} />
                )))}
              </Show>
            </P.Container>
          </P.Container>
