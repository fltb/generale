  ...
  return (
    <Collapse arrow class="border border-base-300 bg-base-100">
      ...
      <CollapseContent>
        ...
        <Show when={!maps.loading} fallback={<div class="flex justify-center py-4"><Spinner /></div>}>
          <Show when={maps() && maps()!.length > 0} fallback={
            <div class="text-center py-4 opacity-50 text-sm">暂无已发布的地图</div>
          }>
                ...
                {(m) => (
                  ...
                  >
                    <div class="w-8 h-8 bg-base-300 rounded shrink-0 overflow-hidden">
                      <img src={mapThumbnailUrl(m.id)} alt="" class="w-full h-full object-cover"
                        onError={(e) => { (e.currentTarget as HTMLImageElement).style.display = 'none'; }} />
                    </div>
                    <div class="min-w-0 flex-1">
                      <div class="text-xs font-medium truncate">{m.name}</div>
                      <div class="text-[10px] opacity-50">{m.authorName} · {m.width}×{m.height} · {m.minPlayers}-{m.maxPlayers}人</div>
                    </div>
                  </button>
                )}
              </For>
