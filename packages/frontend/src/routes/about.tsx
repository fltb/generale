...
export default function About() {
  ...
  return (
    <main class="max-w-3xl mx-auto px-6 py-10 text-sm text-base-content/80 leading-relaxed space-y-4">
      <Title>
        {t("About")} — {PLATFORM_NAME}
      </Title>
      <Meta
        name="description"
        content={`${t("About")} ${PLATFORM_NAME}, ${t("an online multiplayer game platform.")}`}
      />
      ...
      <p>
        {t(
          "{name} is an online multiplayer gaming platform. Free to play, no download required — just a browser and an internet connection.",
          { name: PLATFORM_NAME },
        )}
      </p>
      ...
      <section>
        ...
        <p>
          {t(
            "<strong>{game}</strong> is our first title — a real-time multiplayer territory conquest strategy game inspired by the classic browser game genre. Each match plays out on a pixel grid map where you command armies, capture territories, and outmaneuver opponents.",
            { game: GAME_NAME },
          )}
        </p>
        <p>
          {t(
            "Upcoming games are in development. {name} is designed as a platform for multiple multiplayer titles sharing the same account system and lobby infrastructure.",
            { name: PLATFORM_NAME },
          )}
        </p>
      </section>
      ...
      <section>
        ...
        <p>
          {t(
            "The entire project is open source under the MIT License. You can find the source code, contribute, report issues, or suggest features on {link}.",
            { link: "GitHub" },
          )}
        </p>
      </section>
