...
export default function Home() {
  ...
  return (
    <>
      ...
      <Meta
        name="description"
        content={`${PLATFORM_NAME} — ${t("Play real-time multiplayer strategy games online. Free, no download.")}`}
      />
      <Meta property="og:title" content={`${PLATFORM_NAME} — ${PLATFORM_TAGLINE}`} />
      <Meta
        property="og:description"
        content={t("Play real-time multiplayer strategy games online. Free, no download.")}
      />
      ...
      <section class="max-w-4xl mx-auto px-6 py-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <GameCard
            title={t("General E")}
            description={t(
              "Conquer territories, command armies, and outsmart your opponents in this real-time multiplayer strategy game.",
            )}
      ...
      <footer class="border-t-2 border-base-300 py-6 text-center text-xs text-base-content/40">
        {PLATFORM_NAME} — {PLATFORM_TAGLINE} · © {new Date().getFullYear()}{" "}
        <a href="/terms" class="hover:text-base-content/60">
          {t("Terms")}
        </a>
      </footer>
