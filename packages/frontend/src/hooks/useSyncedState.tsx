...
>({
  ...
  function _onMessage(msg: SyncedStateServerEvent<TState, Custom>) {
    ...
    if (msg.type === SyncedStateServerEventType.ACTION_RESULT) {
      ...
      if (typeof optimisticId === "number" && pendingRequests.has(optimisticId)) {
    ...
    try {
      ...
      if (wsMgr) {
        try {
          ...
          const maybeSub = wsMgr.getOrCreateSub(domain);
          if (maybeSub) {
            sub = maybeSub;
            // attach handlers to existing sub
            ensureSubAndAttach();
            if (sub?.ready) _onOpen();
          }
        } catch {
          // ignore
        }
  ...
  // ---- FIXED connect: create sub, attach handlers, then request open ----
  function connect() {
    try {
      // ensure websocket connection
      if (!wsMgr.isConnected) wsMgr.connect(true);

      // ensure local sub object and attach handlers BEFORE requesting open
      ensureSubAndAttach();

      // request open (add to pendingOpens and send or retry)
      if (autoOpen) {
        try {
          wsMgr.openDomain(domain, context);
          console.debug(`[useSyncedState:${domain}] openDomain requested`, domain, context);
        } catch (e) {
