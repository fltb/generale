...
export class GameInstance implements IBaseInstance<SyncedGameClientActions, SyncedGameServerEvent> {
  ...
  private triggerEndGame(result: GameEndResult) {
    for (const callback of this.onEndGameCallbacks) {
      try {
        ...
        console.error("[GameInstance] onEndGame callback error", e);
      }
    }
  }
  ...
  private ensureSyncEntry(pid: PlayerId) {
    const existing = this.syncData.get(pid);
  ...
  public addPlayer(
    ...
    displace(
      ...
      this.stateSync,
      this.syncData,
      SyncedGameServerEventType.CUSTOM,
  ...
  private handleClientEvent(pid: PlayerId, evt: SyncedGameClientActions) {
    ...
    const evtWithOptId = evt as { optimisticId?: number };
    const optimisticId = evt && typeof evtWithOptId.optimisticId === "number" ? evtWithOptId.optimisticId : undefined;
    if (typeof optimisticId === "number" && synced.lastConfirmedOp >= optimisticId) {
      console.debug(
        `[GameInstance] drop stale evt from ${pid} optimisticId=${optimisticId} lastConfirmedOp=${synced.lastConfirmedOp}`,
      );
      return;
    }
  ...
  private handleSurrender(pid: PlayerId) {
    ...
    if ((this.state.status as GameStatus) === GameStatus.Ended) {
      ...
      const winnerId = winnerTeam?.memberIds[0] ? winnerTeam.memberIds[0] : "";
  ...
  private broadcastGameEnded(): void {
    ...
    for (const conn of this.spectatorConnectors.values()) {
      try {
        ...
        console.warn("[GameInstance] broadcastGameEnded spectator send error", e);
      }
    }
  }
  ...
  public addSpectator(
    ...
    if (sid in this.state.players) {
      ...
      return {
        success: false,
        message: "[GameInstance] player already in game; should connect as player not spectator",
      };
    }
    ...
    displace(
      sid,
      connector,
      this.spectatorConnectors,
      this.spectatorStateSync,
      new Map(),
      SyncedGameServerEventType.CUSTOM,
      SyncedPreGameServerEventPayloadType.DISPLACED,
    );
