...
import { ChatClientToServer, ChatServerToClient } from '@generale/types/src/game/chat';
...
describe('GameChatInstance', () => {
  ...
  it('玩家发送消息应广播给所有玩家并入队', () => {
    chat.addPlayer(player1, 'Alice', connector1);
    let handler1: any;
    (connector1.onMessage as any).mockImplementation(cb => { handler1 = cb; });
  ...
  it('消息队列超限应丢弃最早的', () => {
    chat.addPlayer(player1, 'Alice', connector1);
    let handler: any;
    (connector1.onMessage as any).mockImplementation(cb => { handler = cb; });
    chat.addPlayer(player1, 'Alice', connector1); // 触发 handler 注册
  ...
  it('玩家离开后不会再收到消息', () => {
    ...
    (connector1.onMessage as any).mockImplementation(cb => { handler = cb; });
    chat.addPlayer(player1, 'Alice', connector1);
    handler && handler({ type: 'send', content: 'bye' });
  ...
  it('未加入玩家发消息无效', () => {
    // 不注册 onMessage
    // 直接调用内部方法模拟
    expect(() => chat["handlePlayerMessage"] && chat["handlePlayerMessage"]('ghost', { type: 'send', content: 'xxx' })).not.toThrow();
  });
