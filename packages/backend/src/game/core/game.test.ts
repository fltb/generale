...
import { areAdjacent, handleMove, isAdjacentToPlayer, updateGameState, validateMove } from "./game-utils";
 ...
 */
function createRandomizedState(width = 10, height = 10, playerConfigs: { id: PlayerId; teamId: TeamId }[]): GameState {
...
describe("tick", () => {
  it("applies successful move and advances tick", () => {
    ...
    const queues = {
      p: [{ type: PlayerOperationType.Move, payload: { from: { x: 1, y: 1 }, to: { x: 1, y: 2 }, percentage: 50 } }],
    };
  ...
  it("skips invalid op without consuming", () => {
    ...
    state.players.p = { id: "p", status: PlayerStatus.Playing, teamId: undefined };
    const queues = {
      p: [{ type: PlayerOperationType.Move, payload: { from: { x: 0, y: 0 }, to: { x: 5, y: 5 }, percentage: 10 } }],
    };
    const { state: newState, queue: newQ } = tick(state, queues);
    expect(newQ.p.length).toBe(0);
    expect(newState.tick).toBe(1);
...
describe("Enhanced Multi-Step and Randomized Tests", () => {
  describe("Fixed Multi-Step Scenarios", () => {
    it("1. 一个玩家应占领相邻的无人地块", () => {
      ...
      const queues: PlayerActionQueues = {
        p1: [{ type: PlayerOperationType.Move, payload: { from: { y: 1, x: 1 }, to: { y: 1, x: 2 }, percentage: 50 } }],
      };
    ...
    it("2. 占领王座会导致玩家战败，其地块被转移", () => {
      ...
      const queues: PlayerActionQueues = {
        p1: [
          { type: PlayerOperationType.Move, payload: { from: { y: 1, x: 0 }, to: { y: 1, x: 1 }, percentage: 100 } },
        ],
      };
      const { state: newState } = tick(state, queues);
    ...
    it("3a. 队友之间可以增援兵力", () => {
      ...
      const queues: PlayerActionQueues = {
        p1: [{ type: PlayerOperationType.Move, payload: { from: { y: 1, x: 1 }, to: { y: 2, x: 1 }, percentage: 50 } }],
      };
      const { state: newState } = tick(state, queues);
    ...
    it("6. 攻击沼泽地块(Swamp)会在占领后损失兵力", () => {
      ...
      const queues: PlayerActionQueues = {
        p1: [
          { type: PlayerOperationType.Move, payload: { from: { y: 0, x: 0 }, to: { y: 1, x: 0 }, percentage: 100 } },
        ],
      };
  ...
  it("7. Vision (mask) should expand after capturing new territory", () => {
    ...
    const queues: PlayerActionQueues = {
      p1: [{ type: PlayerOperationType.Move, payload: { from: { x: 1, y: 1 }, to: { x: 1, y: 2 }, percentage: 100 } }],
    };
    state = tick(state, queues).state;
  ...
  it("8. Team vision (mask) reveals teammate`s actions and territory", () => {
    ...
    const maskedState = mask(state, "p1");
  ...
  it("9. An invalid move in a queue should be rejected, not halting the game", () => {
    ...
    const queues: PlayerActionQueues = {
      p1: [{ type: PlayerOperationType.Move, payload: { from: { x: 0, y: 0 }, to: { x: 5, y: 5 }, percentage: 50 } }],
    };
            ...
            if (ownedTiles.length > 0) {
              ...
              for (let dy = -1; dy <= 1; dy++)
                for (let dx = -1; dx <= 1; dx++) {
                  ...
                  if (
                    to.x >= 0 &&
                    to.x < state.map.width &&
                    to.y >= 0 &&
                    to.y < state.map.height &&
                    state.map.tiles[to.y][to.x].type !== TileType.Mountain
                  ) {
