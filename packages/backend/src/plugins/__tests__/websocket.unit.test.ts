...
describe("WebSocket单元测试", () => {
  ...
  describe("域名处理器注册测试", () => {
    ...
    it("应该在重复注册时显示警告", () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith("Domain handler for 'test-domain' already exists, overwriting");
  ...
  describe("域名处理器功能测试", () => {
    ...
    it("onClose处理器应该接收连接ID、代码和原因", () => {
      ...
      expect(onClose).toHaveBeenCalledWith("test-connection-id", 1000, "Normal closure");
    });
  ...
  describe("消息格式验证测试", () => {
    it("应该正确处理各种消息类型", () => {
      const onMessage = vi.fn().mockImplementation((_connectionId, payload) => {
