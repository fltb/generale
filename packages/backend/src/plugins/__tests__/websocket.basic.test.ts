...
// 模拟WebSocket连接管理器
import { domainHandlers, SubConnectorImpl, unregisterDomainHandler } from "../websocket";

class MockWebSocketConnectionManager {
  private connectionId: string;
  private isConnected: boolean = true;
  private subConnectors = new Map<string, SubConnectorImpl<unknown, unknown>>();

  constructor(connectionId: string) {
    this.connectionId = connectionId;
  }

  openSubConnector(domain: string, config: Record<string, unknown>) {
  ...
  routeMessage(domain: string, payload: unknown) {
...
describe("WebSocket基础连接测试", () => {
  ...
  describe("连接生命周期测试", () => {
    ...
    it("应该能够处理多个并发连接", () => {
      // 模拟多个连接
      const connections = Array.from({ length: 5 }, (_, i) => new MockWebSocketConnectionManager(`connection-${i}`));
