...
import {
  type DomainHandler,
  SubConnectorImpl as MockSubConnector,
...
describe("WebSocket Domain Handler Functionality", () => {
  ...
  let mockHandler: {
    onOpen: (connId: string, config: Record<string, unknown>) => void;
    onMessage: (connId: string, payload: unknown) => void;
    onClose: (connId: string, code?: number, reason?: string) => void;
    onDisconnect: (connId: string, err?: Error) => void;
  ...
  beforeEach(() => {
    ...
    mockHandler = {
      onOpen: (connId: string, config: Record<string, unknown>) => {
        receivedMessages.push({ type: "open", connId, config });
      },
      onMessage: (connId: string, payload: unknown) => {
  ...
  describe("Domain Handler Registration", () => {
    it("should register and unregister domain handlers", () => {
      // 注册域名处理器
      registerDomainHandler("test-domain", mockHandler as unknown as DomainHandler);
  ...
  describe("Domain Handler Callbacks", () => {
    beforeEach(() => {
      registerDomainHandler("callback-test", mockHandler as unknown as DomainHandler);
    });
    ...
    it("should handle onMessage callback", () => {
      ...
      const handler: DomainHandler = (connector) => {
        connector.onClientMessage((msg: unknown) => {
          receivedMessages.push({
            type: "message",
            connId: mockConnectionId,
            payload: msg,
    ...
    it("should handle multiple domains independently", () => {
      ...
      const gameHandler: DomainHandler = (connector) => {
        connector.onClientMessage((payload) => {
          gameMessages.push({ domain: "game", payload: payload as Record<string, unknown> });
        });
      };
      const chatHandler: DomainHandler = (connector) => {
        connector.onClientMessage((payload) => {
          chatMessages.push({ domain: "chat", payload: payload as Record<string, unknown> });
        });
      };
      ...
      const gameMsg = gameMessages[0] as { domain: string; payload: { action: string } };
      expect(gameMsg.payload.action).toBe("move");

      expect(chatMessages).toHaveLength(1);
      const chatMsg = chatMessages[0] as { domain: string; payload: { message: string } };
      expect(chatMsg.payload.message).toBe("Hello!");
    });

    it("should handle complex game scenario", () => {
      const gameState = {
        players: new Map<string, { name: string; ready: boolean; position: { x: number; y: number } }>(),
        messages: [] as unknown[],
      };

      const gameHandler: DomainHandler = (connector) => {
        connector.onOpen(() => {
          const ctx = connector.context as unknown as { playerName: string };
          ...
          gameState.messages.push({ type: "player_joined" as const, connId, config: connector.context });
        });
        connector.onClientMessage((payload) => {
          const ctx = connector.context as unknown as { playerName: string };
          const connId = `${connector.domain}-${ctx.playerName || ""}`;
          ...
          switch (msg.action) {
            case "ready":
              player.ready = true;
              gameState.messages.push({ type: "player_ready" as const, connId });
              break;
            case "move":
              player.position = msg.position ?? player.position;
              gameState.messages.push({ type: "player_moved" as const, connId, position: msg.position });
              break;
          }
        });
        connector.onClose(() => {
          ...
          gameState.messages.push({ type: "player_left" as const, connId });
        });
      };
      ...
      // 验证消息历史
      const msgs = gameState.messages as Array<{ type: string; connId: string }>;
      expect(msgs).toHaveLength(7);
      expect(msgs[0].type).toBe("player_joined");
      expect(msgs[1].type).toBe("player_joined");
      expect(msgs[2].type).toBe("player_ready");
      expect(msgs[3].type).toBe("player_ready");
      expect(msgs[4].type).toBe("player_moved");
      expect(msgs[5].type).toBe("player_moved");
      expect(msgs[6].type).toBe("player_left");
    });
  });
  ...
  describe("Performance Considerations", () => {
    ...
    it("should handle high-frequency handler calls", () => {
      ...
      const highFreqHandler = {
        onMessage: (_connectionId: string, _payload: unknown) => {
          messageCount++;
        },
      };

      registerDomainHandler("high-freq", highFreqHandler as unknown as DomainHandler);
