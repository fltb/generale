...
import {
  ...
  WebSocketMessage,
  domainHandlers,
} from '../websocket'; // Adjust the import path as necessary

// Mock SubConnector for testing domain logic
class MockSubConnector implements SubConnector<any, any, any> {
  public _ready = true;
  ...
  send = (payload: any) => this.manager.sendRaw({ domain: this.domain, type: 'message', payload });
  close = (code?: number, reason?: string) => this.manager.closeSubConnector(this.domain, code, reason);

  onOpen = (cb: () => any) => this.openCallbacks.push(cb);
  ...
  onClientMessage = (cb: (payload: any) => any) => this.messageCallbacks.push(cb);
  
  triggerOpen = () => this.openCallbacks.forEach(cb => cb());
  triggerClose = (code?: number, reason?: string) => {
    this._ready = false;
    this._explicitlyClosed = true;
    this.closeCallbacks.forEach(cb => cb(code, reason));
  }
  triggerMessage = (payload: any) => this.messageCallbacks.forEach(cb => cb(payload));
  triggerDisconnect = () => {
    this._ready = false;
...
class MockConnectionManager {
  ...
  constructor(public readonly connectionId: string, public readonly userId: string) {}
