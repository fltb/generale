...
function buildSnapshotForClient(allGames: GameInfoSuccessResp["data"][], ctxFilters?: Partial<ListGamesQuery> | null) {
  ...
  if (q.offset !== undefined || q.limit !== undefined) {
    ...
    return {
      items: page.items,
      meta: { total: page.total, offset: page.offset, limit: page.limit, hasMore: page.hasMore },
    };
  }
...
export class GeneraleManager {
  ...
  public onRoomCreated(cb: (gameId: GameId) => void): () => void {
    ...
    return () => {
      const i = this.roomCreatedCallbacks.indexOf(cb);
      if (i >= 0) this.roomCreatedCallbacks.splice(i, 1);
    };
  }
  public onRoomDeleted(cb: (gameId: GameId) => void): () => void {
    this.roomDeletedCallbacks.push(cb);
    return () => {
      const i = this.roomDeletedCallbacks.indexOf(cb);
      if (i >= 0) this.roomDeletedCallbacks.splice(i, 1);
    };
  }
  public onRoomUpdated(cb: (gameId: GameId) => void): () => void {
    this.roomUpdatedCallbacks.push(cb);
    return () => {
      const i = this.roomUpdatedCallbacks.indexOf(cb);
      if (i >= 0) this.roomUpdatedCallbacks.splice(i, 1);
    };
  }

  private emitRoomCreated(gameId: GameId) {
    for (const cb of this.roomCreatedCallbacks) {
      try {
        cb(gameId);
      } catch (err) {
        console.error("[GSM] emitRoomCreated error", err);
      }
    }
  }
  private emitRoomDeleted(gameId: GameId) {
    for (const cb of this.roomDeletedCallbacks) {
      try {
        cb(gameId);
      } catch (err) {
        console.error("[GSM] emitRoomDeleted error", err);
      }
    }
  }
  private emitRoomUpdated(gameId: GameId) {
    for (const cb of this.roomUpdatedCallbacks) {
      try {
        cb(gameId);
      } catch (err) {
        console.error("[GSM] emitRoomUpdated error", err);
      }
    }
  }
