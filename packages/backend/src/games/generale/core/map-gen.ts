...
export async function generateMap(
  ...
  const ms = mapSetting as PreGameRoomState["mapSetting"] & {
    width?: number;
    height?: number;
    tileFrequency?: Partial<Record<TileType, number>>;
    customMapId?: string;
  };
  const width = ms.width || 20;
  const height = ms.height || 20;
  const type = mapSetting.type;
  const tileFrequency = ms.tileFrequency || {};
  const customMapId = ms.customMapId as string | undefined;
    ...
    if (customData) {
      ...
      for (let y = 0; y < actualHeight; y++) {
        ...
        for (let x = 0; x < actualWidth; x++) {
          row[x] = { type: TileType.Plain, ownerId: null, army: 0 };
        }
      }
      for (let y = 0; y < Math.min(actualHeight, customData.length); y++) {
        const customRow = customData[y];
        if (!customRow) continue;
        for (let x = 0; x < Math.min(actualWidth, customRow.length); x++) {
          const ct = customRow[x];
          if (!ct) continue;
  ...
  if (customMapId) {
    ...
    console.log(
      `[map-gen] Custom map ${customMapId}: ${throneCoords.length} pre-placed thrones, ${remaining.length} random fallback`,
    );
  } else {
...
function generateRandomMap(
  ...
  for (const k of Object.keys(frequency)) {
    normalized[k] = ((frequency as Record<string, number>)[k] ?? 0) / totalFreq;
  }
    ...
    for (let x = 0; x < width; x++) {
      ...
      for (const [tileType, freq] of Object.entries(normalized)) {
        ...
        if (rand <= cumulative) {
          const tile = row[x];
          if (!tile) continue;
          tile.type = tileType as TileType;
          if (tileType === TileType.Barracks) {
            tile.army = Math.floor(Math.random() * 5) + 1;
          } else {
            tile.army = 0;
          }
          break;
...
function adjustBarracksArmies(tiles: GameMap["tiles"], width: number, height: number): void {
  const MIN = 20;
  const MAX = 40;
...
function randomizeNear(
  ...
  for (let t = 0; t < tries; t++) {
    ...
    if (rx >= margin && rx < width - margin && ry >= margin && ry < height - margin) {
      const tileRow = tiles[ry];
      if (!tileRow) continue;
      const tile = tileRow[rx];
      if (!tile) continue;
  ...
  // 最后兜底返回中心
  return {
    x: Math.max(margin, Math.min(cx, width - margin - 1)),
    y: Math.max(margin, Math.min(cy, height - margin - 1)),
  };
}

/** 收集平原和兵营作为出生点候选 */
function getPlainAndBarracksTiles(
  width: number,
  height: number,
  tiles: GameMap["tiles"],
  margin = 2,
