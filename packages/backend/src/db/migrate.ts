...
function sealExistingMigrations(dbPath: string) {
  ...
  for (const entry of journal.entries) {
    ...
    if (!tables || tables.length === 0) {
      // migration with no newly-created tables — seal only if __drizzle_migrations was just created
      if ((sqlite.prepare(`SELECT COUNT(*) as c FROM __drizzle_migrations`).get() as any)?.c === 0) {
        sqlite.prepare(`INSERT INTO __drizzle_migrations (hash, created_at) VALUES (?, ?)`).run(hash, entry.when);
        sealed++;
      }
      continue;
    }
    if (tables.every(t => sqlite.prepare(`SELECT name FROM sqlite_master WHERE type='table' AND name='${t}'`).get())) {
      sqlite.prepare(`INSERT INTO __drizzle_migrations (hash, created_at) VALUES (?, ?)`).run(hash, entry.when);
      sealed++;
    }
  }
