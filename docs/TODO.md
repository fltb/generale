...
现在这个网站的框架已经准备好了，CI/CD 和测试都有了，那么已经可以正式到线上开发。但是还需要做一些准备

### 在线部署和更新准备

我现在有域名和服务器，理论上可以直接配合 github actions 完成自动化集成部署，作为“内测版本”运行项目，只要 push 到 main 就自己部署新版本，开发自己用 dev 分支。

那么我们现在就需要对此做好项目上的准备：确保项目的一定稳定性，确保后续拓展的功能不会有非常破坏性的更新，而且保证上线的相对稳定。

现在就需要检查目前 TODO 里面的计划是否会有大影响的部分，对他们可能会破坏的地方留意，在线有架构上留一点余量

然后还需要保证能够尽量无痛更新，因为需要冷启动

然后因为是在线部署的东西，我们不希望给用户太多垃圾输出和测试数据在 console 里面。最好也要保证前端的性能。

### 用户体验准备

需要检查用户的 UX 是否完备（游戏的外观与功能优化）部分也有提到。

但是我还需要一些大的点：

比如 i18n 的引入。

比如主页进来之后清楚地表明这是一个游戏网站

比如美术方面的优化

比如 SEO 和最近大家喜欢的 LLM 优化。

然后确保用户不会乱点点坏东西的同时也不会不知道点什么

最好还要把后续待完成的 TODO 也放在这部分思考一下

## 游戏的外观与功能优化

#### 官方匹配模式和 General E 内部导航 UX 优化

前端也要读取用户的语言设置来改变语言设置

在 general E 下面点击用户不显示 logout/profile 下拉栏目。这个右上角组件需要统一 

取消 general E nav 右侧的 <-platform，改为在 Gerneral E 标题左侧加一个 <- 表示可以返回上一级

为了方便直观，我们在除了顶部的 Nav 之外，还需要一些醒目的方式表示导航跳转，最好的方式是做一个左侧栏，然后给每个带图标

我们可能需要引入官方的匹配模式。目前就官方 FFA 和 2x4 吧
