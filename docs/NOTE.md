...
- [ ] 需要处理 gameover 事件，在触发事件之后，后端自动断开 game 的 subconnection, 切换到 pregame 状态，等待客户端重新连接。前端则显示游戏结束，允许玩家重新跳转到 pregame 或者退出。
...
- [ ] 前后端应该给玩家加入切换队伍的选项。应该至少存在两个队伍，且后端内部给队伍命名，前端不需要显示准确的内部名。
...
- [ ] room list 的 websocket 事件同步，确保实时同步信息
- [ ]  