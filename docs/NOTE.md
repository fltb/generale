...
- [ ] 修改默认头像的风格，更符合现在的像素风
- [ ] 地图目前是固定大小的画布，应该改成可以在一块画布上拖动和缩放，方便大地图。现在的大地图会溢出画布导致看不到有部分画面
